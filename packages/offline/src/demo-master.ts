...
export async function snapshotMasterData(): Promise<SnapshotResult> {
  ...
  try {
    ...
    // Write into demo IndexedDB
    await Promise.all([
      upsertDemoProducts(products),
      upsertDemoVariants(variants),
      upsertDemoModifiers(modifiers),
      upsertDemoPromotions(promotions),
      upsertDemoTaxRates(taxRates),
    ]);
