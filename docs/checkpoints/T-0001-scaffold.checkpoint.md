# Checkpoint: T-0001 — Scaffold pnpm workspace + apps/web baseline + packages skeleton
...
Membuat fondasi monorepo pnpm workspace sesuai SYSTEM-DESIGN §6, lengkap dengan baseline untuk keempat app dan semua package. Semua app dan package harus bisa di-typecheck tanpa error.
...
- [x] `apps/web` — Next.js 15 baseline (App Router) dengan layout minimal
- [x] `apps/site` — Next.js 15 baseline (App Router) dengan layout minimal
- [x] `apps/mcp` — Hono baseline dengan entry point
- [x] `apps/worker` — Node baseline dengan entry point
- [x] `packages/db` — skeleton (index.ts, client.ts baseline)
...
3. [x] apps/web — Next.js 15 baseline
4. [x] apps/site — Next.js 15 baseline
5. [x] apps/mcp — Hono baseline
6. [x] apps/worker — Node baseline
...
- apps/web: Next.js 15 baseline with layout, page, (auth)/login, (dash)/layout
- apps/site: Next.js 15 baseline with layout, page, (public), (member), api dirs
- apps/mcp: Hono server with /healthz endpoint + auth baseline
- apps/worker: Node entry with SIGTERM handler
- packages/db: Drizzle client (Neon), schema baseline (auth, accounting, inventory, audit), seed baseline
