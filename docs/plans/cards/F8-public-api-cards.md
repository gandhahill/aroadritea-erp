...
> **STATUS (2026-06-10, T-0293 — dibawa maju atas permintaan user "kerjakan sekarang"; interpretasi Scalar DIKONFIRMASI user)**: F8.1 (ADR-0017 Accepted) + F8.2 (fondasi `/api/v1` + 3 endpoint read: products, stock, reports/daily-summary; auth Bearer `api_tokens`, rate limit per token, audit `source=public_api`, error envelope seragam) + sebagian F8.5 (Scalar UI di `/docs` via CDN + runbook `docs/runbook/public-api-onboarding.md`) **SUDAH SELESAI**. Deviasi terdokumentasi: spec OpenAPI didefinisikan di kode + Scalar via CDN (bukan `@hono/zod-openapi`/`@scalar/hono-api-reference`) demi menghindari peer dependency zod v3 — lihat ADR-0017. SISA: F8.3 (endpoint read lanjutan), F8.4 (mutasi idempoten), F8.5 (tautan dari Settings API Token), F8.6 (sapuan keamanan API + simulasi onboarding buta + daftar ke pentest F6.4).
