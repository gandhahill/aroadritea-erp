...
# SD Â§35.2 Scenario 8: DB connection drop â†’ app reconnects â‰¤ 5s,
#   no requests lost.
#
...
echo "  SD Â§35.2 â€” Expected: reconnect â‰¤ 5s, no request lost"
echo "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"
...
while [ $ELAPSED -lt $MAX_RECOVERY_WAIT ]; do
  ...
  if echo "$HEALTH_RESPONSE" | grep -qi "ok\|healthy"; then
    ...
    if [ $RECOVERY_TIME -le 5 ]; then
      echo "  OK PASS: DB reconnected in ${RECOVERY_TIME}s (target: â‰¤ 5s)"
    elif [ $RECOVERY_TIME -le 15 ]; then
      echo "  WARNING WARN: DB reconnected in ${RECOVERY_TIME}s (target: â‰¤ 5s)"
    else
