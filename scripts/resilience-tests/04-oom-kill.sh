...
if [ -z "$CONTAINER_ID" ]; then
  echo "  ERROR FAIL: $SERVICE container not found."
  exit 1
fi
echo "  OK $SERVICE is running (ID: ${CONTAINER_ID:0:12})"
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  if curl -sf "$HEALTHZ_URL" > /dev/null 2>&1; then
    ...
    if [ $RECOVERY_TIME -le 30 ]; then
      echo "  OK PASS: Auto-recovery time = ${RECOVERY_TIME}s (target: ≤ 30s)"
    elif [ $RECOVERY_TIME -le 120 ]; then
      echo "  WARNING WARN: Auto-recovery time = ${RECOVERY_TIME}s (within RTO 120s)"
    else
      echo "  ERROR FAIL: Auto-recovery time = ${RECOVERY_TIME}s (exceeds RTO 120s)"
    fi
    echo "═══════════════════════════════════════════════════════════"
    ...
    if [ "$NEW_CONTAINER_ID" != "$CONTAINER_ID" ]; then
      echo "  OK New container spawned (ID: ${NEW_CONTAINER_ID:0:12})"
    else
      echo "  WARNING Same container ID - may have been restarted in-place"
    fi
    exit 0
  fi
  sleep 1
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ERROR FAIL: $SERVICE did not auto-restart within ${MAX_WAIT}s"
