...
| G15 | Increment `promotions.usageCount` setelah promo diterapkan ke sale — `usageLimit` saat ini tidak pernah mengikat | **P1** | S | Ditemukan saat kerjakan G3a. `evaluator.ts:71-78` cek `usageCount >= usageLimit`, tapi grep `usageCount` di `create-sale.ts` nol hasil. Lihat Finding 5. |
