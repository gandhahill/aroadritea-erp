...
|----|-------|-------|---------|-------------|--------|------|
| T-0299 | F&B ERP gap audit vs Odoo/ERPNext: presence + LOGIC dual-lens (POS/Kitchen/Promotion focus) | Claude Sonnet 4.6 | 2026-06-11 | 2026-06-12 | 🟨 IN_PROGRESS | Per permintaan Lintang: brainstorm fitur+logika F&B ERP from-scratch (`docs/benchmark/fnb-erp-feature-checklist.md`, ~260 item §0-§17, dibuat tanpa bias state repo) lalu audit ground-truth dual-lens (presence DAN logic correctness). Deliverable: `docs/benchmark/fnb-erp-gap-analysis.md`. §3 (POS/Restaurant) full deep-audit; 5 Headline Findings P0/P1: (1) modifier sugar/ice/topping — schema `product_modifier_groups/options/links` 100% lengkap tapi `pos/product-search.tsx` `handleAddProduct` TIDAK PERNAH set `modifierJson`, tidak ada picker UI sama sekali (Finding 1 → G1); (2) promo `buy_x_get_y`/`free_item`/`complimentary` silent no-op di `promotion/evaluator.ts:133-136` (`continue` tanpa error) — manager bisa aktifkan promo BOGO dan TIDAK ADA YANG TERJADI, tanpa log/audit (Finding 2 → G3a ✅ DONE T-0300, G3b pending ADR); (3) KDS: `kds-service.ts` (399 baris, 26 test) + `display-service.ts` (169 baris, 11 test) 100% selesai+teruji+auto-wired (`create-sale.ts:1390` queue tiap sale), permission `kitchen.view` ter-seed, TAPI nol route/halaman/nav entry — staf dapur tidak punya layar sama sekali (Finding 3 → G2); (4) tabel `reservations` 100% dorman, nol referensi (Finding 4 → G6, perlu keputusan Lintang: dine-in/table service relevan atau tidak); (5) promo `usageLimit` dicek di evaluator tapi `usageCount` tidak pernah di-increment di `create-sale.ts` → limit tidak pernah mengikat (Finding 5 → G15). §0-2,4-17 first-pass presence (Part C) + backlog G1-G15 (Part D) ditambahkan ke bawah; G3a ✅ DONE T-0300, G4 ✅ DONE T-0301 ("86" toggle produk), G15 ✅ DONE T-0302 (usageCount increment), prioritas berikutnya: G2 (KDS UI), G1 (modifier picker, perlu ADR kecil). Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
...
| G2 | KDS staff board + customer display UI — full Finding 3 remediation | **P0** | L | Backend sudah selesai; murni UI+route+nav+SSE. |
