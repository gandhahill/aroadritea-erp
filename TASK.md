...
|----|-------|-------|---------|-------------|--------|------|
| T-0299 | F&B ERP gap audit vs Odoo/ERPNext: presence + LOGIC dual-lens (POS/Kitchen/Promotion focus) | Claude Sonnet 4.6 | 2026-06-11 | 2026-06-13 | 🟨 IN_PROGRESS | Per permintaan Lintang: brainstorm fitur+logika F&B ERP from-scratch (`docs/benchmark/fnb-erp-feature-checklist.md`, ~260 item §0-§17, dibuat tanpa bias state repo) lalu audit ground-truth dual-lens (presence DAN logic correctness). Deliverable: `docs/benchmark/fnb-erp-gap-analysis.md`. §3 (POS/Restaurant) full deep-audit; 5 Headline Findings P0/P1: (1) modifier sugar/ice/topping — schema `product_modifier_groups/options/links` 100% lengkap tapi `pos/product-search.tsx` `handleAddProduct` TIDAK PERNAH set `modifierJson`, tidak ada picker UI sama sekali (Finding 1 → G1 ✅ DONE T-0304); (2) promo `buy_x_get_y`/`free_item`/`complimentary` silent no-op di `promotion/evaluator.ts:133-136` (`continue` tanpa error) — manager bisa aktifkan promo BOGO dan TIDAK ADA YANG TERJADI, tanpa log/audit (Finding 2 → G3a ✅ DONE T-0300, G3b pending ADR); (3) KDS: `kds-service.ts` (399 baris, 26 test) + `display-service.ts` (169 baris, 11 test) 100% selesai+teruji+auto-wired (`create-sale.ts:1390` queue tiap sale), permission `kitchen.view` ter-seed, TAPI nol route/halaman/nav entry — staf dapur tidak punya layar sama sekali (Finding 3 → G2 ✅ DONE T-0303); (4) tabel `reservations` 100% dorman, nol referensi (Finding 4 → G6, perlu keputusan Lintang: dine-in/table service relevan atau tidak); (5) promo `usageLimit` dicek di evaluator tapi `usageCount` tidak pernah di-increment di `create-sale.ts` → limit tidak pernah mengikat (Finding 5 → G15). §0-2,4-17 first-pass presence (Part C) + backlog G1-G15 (Part D) ditambahkan ke bawah; G3a ✅ DONE T-0300, G4 ✅ DONE T-0301 ("86" toggle produk), G15 ✅ DONE T-0302 (usageCount increment), G2 ✅ DONE T-0303 (KDS staff board `/kitchen` + customer display `/kitchen-display/[locationId]` + SSE + nav + i18n), G1 ✅ DONE T-0304 (ADR-0019 `groupRole` + canonical `ModifierSelection[]` + picker UI + KDS/Naixer/label/offline sync), prioritas berikutnya: lanjutkan audit ⬜ §0-2/4-17 (Part C) lalu 4 keputusan bisnis Lintang. Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
| T-0295 | Stock UOM normalization at every stock_levels write path + informative deduction errors | Claude Fable 5 | 2026-06-11 12:13 WIB | 2026-06-11 12:36 WIB | 🟩 DONE | Follow-up insiden prod 2026-06-11 (`pos.createSale.ingredientUomMismatch` di MLI; data sudah dibetulkan manual + audit_log). Helper `toProductUom` di uom-service; GRN/adjustment/transfer/opname/import/seed kini menulis `stock_levels` dalam `products.uom` (konversi via `uom_conversions` atau ditolak); error deduksi POS kini menyebut nama bahan + satuan (i18n en/id/zh) via `describeDeductError`. Verified: typecheck services/db/web, biome touched-files clean, services tests 665/665, i18n parity. Backlog: UI manajemen `uom_conversions`. Checkpoint: `docs/checkpoints/T-0295-stock-uom-normalization.checkpoint.md`. |
| T-0294 | Product direction: Odoo-like FnB ERP platform | Codex | 2026-06-10 23:33 WIB | 2026-06-10 23:33 WIB | DONE | User clarified target: ERP must be as complete/flexible as Odoo while remaining FnB-specific. Captured in SoT §18.7, SD P17/§38, ADR-0018, and master plan framing/cards. Checkpoint: `docs/checkpoints/T-0294-odoo-like-fnb-erp-platform.checkpoint.md`. |
| T-0293 | Public REST API /api/v1 + Scalar docs (F8.1/F8.2 + partial F8.5, brought forward) | Claude Opus 4.8 | 2026-06-10 23:05 WIB | 2026-06-10 23:20 WIB | DONE | User confirmed "Scalar — kerjakan sekarang". Built `/api/v1` on apps/mcp Hono: Bearer auth (api_tokens) → per-token rate limit → permission engine (`can`) → audit (`source=public_api`); 3 read endpoints (products, stock, reports/daily-summary) reusing services; OpenAPI 3.1 spec in code at `/api/v1/openapi.json`; Scalar UI at `/docs` (CDN embed, no new dep — avoids zod v3 peer). ADR-0017 + onboarding runbook. 11 mcp tests + mcp typecheck PASS; Biome clean. Remaining F8.3/F8.4/F8.6 deferred. Checkpoint: `docs/checkpoints/T-0293-public-rest-api-scalar.checkpoint.md`. |
| T-0292 | HR fixes: encrypt NIK on employee update, whistleblower submission notification, attendance summary late-minutes total | Claude Opus 4.8 | 2026-06-10 23:00 WIB | 2026-06-10 23:05 WIB | DONE | (1) `update-employee.ts` stored NIK plaintext (UU PDP / CLAUDE.md §5.5) — now `encryptPii` like create. (2) `submitWhistleblowerReport` now fans out an anonymity-safe in-app notification to `hr.whistleblower.read` holders. (3) `attendance-summary-service` `totalLateMinutes` SUM returned a string → client `+` concatenated it (`005300000310160m`); cast `::int` at source. Checkpoint: `docs/checkpoints/T-0292-hr-nik-whistleblower-attendance.checkpoint.md`. |
| T-0291 | SAK EP reporting/tax code gap closure (current/non-current BS, P&L classification, PPh23 no-NPWP surcharge) | Claude Opus 4.8 | 2026-06-10 22:30 WIB | 2026-06-10 22:45 WIB | DONE | Closed the "Partial" code gaps left by T-0290: (1) balance sheet current vs non-current grouping (SAK EP Bab 4), (2) P&L separate other income/finance costs/income tax (SAK EP Bab 5; `7-4100`→subtype `income_tax`), (3) PPh 23 doubles rate when vendor has no NPWP (UU PPh Ps.23(1a)). Typecheck services/db/mcp/web PASS; 63 reporting+tax tests PASS; Biome PASS. Flagged pre-existing MCP bigint serialization bug as follow-up. Checkpoint: `docs/checkpoints/T-0291-sak-ep-reporting-tax-gaps.checkpoint.md`. |
| T-0289 | F0.1 - CI master branch and i18n parity job | Codex | 2026-06-10 19:51 WIB | 2026-06-10 23:02 WIB | DONE | CI now runs on `master`, i18n parity is enforced after tests, package-level typecheck/test observability is in place, and latest `master` run `27288426676` completed success. Commits through `a14bb95`; checkpoint: `docs/checkpoints/T-0289-f0-1-ci-master-i18n.checkpoint.md`. |
| T-0288 | Tambah fase F7 (app native POS + silent printing) & F8 (API publik + Scalar) ke master plan | Claude Fable 5 | 2026-06-10 18:10 WIB | 2026-06-10 18:35 WIB | DONE | Permintaan user: ERP installable native di Android & Windows untuk silent print USB/Bluetooth + dokumentasi API pihak ketiga. F7 = implementasi ADR-0015 (shell Tauri 2 surface POS, builder ESC/POS, 4 adapter transport, `pos_printer_configs`, fallback browser) — 9 kartu di `docs/plans/cards/F7-native-print-cards.md`, dieksekusi setelah gerbang F3. F8 = REST `/api/v1` menumpang `apps/mcp` (Hono + zod-openapi, auth `api_tokens`, idempoten, rate limit) + docs **Scalar** — 6 kartu di `docs/plans/cards/F8-public-api-cards.md`, sebelum penutupan F6 supaya ikut pentest. Catatan: "Scala" ditafsirkan **Scalar** (tool docs OpenAPI); bila maksudnya bahasa Scala, kartu F8.1 BLOCKED. Urutan fase kini = urutan baris tabel §3 master plan: F0→F1→F2→F3→F7→F4→F5→F8→F6. ADR-0015 (draft sesi sebelumnya) ikut di-commit berstatus Proposed. Checkpoint: `docs/checkpoints/T-0288-add-f7-f8-phases.checkpoint.md`. |
...
| ID | Title | Owner | Completed | Commit | Note |
|----|-------|-------|-----------|--------|------|
| T-0296 | POS drafts: save-as-draft + auto-delete on post (manual sales & consumed ingredients) | Claude Fable 5 | 2026-06-11 | `ddb79b4` | Tabel baru `pos_drafts` (migrasi 0043) + service drafts (pos.transact, audit) + `DraftsPanel` bersama di `/pos/manual-sales` dan `/consumed`. Draft tersimpan server-side; posting sukses menghapus draft yang dimuat. Tests 665/665 PASS, typecheck/lint/i18n parity clean. Checkpoint: `docs/checkpoints/T-0296-pos-drafts.checkpoint.md`. |
| T-0297 | UOM conversions management UI (/inventory/uom-conversions) | Claude Fable 5 | 2026-06-11 | `ddb79b4`, `427c64c` | Service CRUD `uom_conversions` (global & per-produk, tolak duplikat dua arah, hard delete + audit) + halaman baru gate `inventory.product` + sidebar/nav-access + i18n ×3. Bonus: `convertQty` kini menormalkan satuan & memfilter `deleted_at`. Tanpa migrasi. Revisi lanjutan: field satuan asal/tujuan jadi dropdown `PRODUCT_UOM_OPTIONS` (sama seperti dropdown produk), bukan input bebas — cegah mismatch satuan. Checkpoint: `docs/checkpoints/T-0297-uom-conversions-ui.checkpoint.md`. |
| T-0298 | MCP tools for uom_conversions CRUD (CLAUDE.md §6 follow-up to T-0297) | Claude Sonnet 4.6 | 2026-06-11 | `7cc689a` | `inventory.list_uom_conversions`, `inventory.upsert_uom_conversion`, `inventory.delete_uom_conversion` di `apps/mcp/src/tools/phase2.ts`, mengikuti pola `inventory.upsert_category`/`inventory.delete_category` (dynamic import + `serializeResult`); delete pakai `requireConfirmation` (hard delete). typecheck + mcp tests + biome touched-files PASS. Checkpoint: `docs/checkpoints/T-0298-mcp-uom-conversions.checkpoint.md`. |
| T-0300 | Promotion evaluator: implement `buy_x_get_y` + `free_item` as line-level discounts + tests (G3a, follow-up to T-0299 audit Finding 2) | Claude Sonnet 4.6 | 2026-06-12 | `de4952f` | New `applyGetItemBenefit()` in `packages/services/src/promotion/evaluator.ts` computes line-level discounts (`appliesTo: 'line'`, `lineId`) for `buy_x_get_y` (gated on `buyProductId`/`buyQty` present in cart) and `free_item` (discounts up to `getQty` units of `getProductId` by `discountBps`, default 100%); both reuse the existing stacking/usage logic. `complimentary` (G3b) remains a documented no-op pending its ADR. New `packages/services/tests/promotion-evaluator.test.ts` (13 tests: percent/fixed regression, BOGO met/not-met/absent, free_item cap/partial-bps/absent, complimentary no-op, usageLimit gating, line+order stacking) — 678/678 services tests PASS, typecheck clean. Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
| T-0301 | "86" product availability toggle in POS (G4, follow-up to T-0299 audit) | Claude Sonnet 4.6 | 2026-06-12 | `0111183` | New `products.is_available`/`is_86d_at` columns (migration `0044_fine_orphan.sql`, not yet applied to any DB) + `packages/services/src/inventory/set-product-availability.ts` (`setProductAvailability`, permission `inventory.product.update`, audit action `update`, before/after `isAvailable`/`is86dAt`). `pos/actions.ts`: `fetchProducts` now returns `isAvailable`/`canToggleAvailability` and exposes `setProductAvailabilityAction`. `product-search.tsx`: eye/eye-slash toggle button (only for users with `inventory.product.update`), 86'd products greyed out + "Unavailable today" badge + disabled add/variant buttons, inline error banner on toggle failure. New MCP tool `inventory.set_product_availability` in `apps/mcp/src/tools/phase2.ts`. i18n ×3 (`pos.unavailableToday`/`markUnavailableToday`/`markAvailableAgain`/`toggleAvailabilityFailed`). No new test file (consistent with sibling `deactivateProduct`/`updateProduct`, which are also untested DB-backed mutations — CLAUDE.md forbids mocking `@erp/db`). Verified: typecheck packages/db + packages/services + apps/web + apps/mcp clean, `lint:permissions` PASS, scoped Biome clean, i18n JSON valid. Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
| T-0302 | Increment `promotions.usageCount` after a sale applies a promotion (G15, follow-up to T-0299 audit Finding 5) | Claude Sonnet 4.6 | 2026-06-12 | `f7d44be` | `packages/services/src/pos/create-sale.ts`: after journal posting + idempotency save (past every remaining rollback point), collects the distinct `promotionId`s from `promoResult.appliedPromotions` and runs `UPDATE promotions SET usage_count = usage_count + 1 WHERE id IN (...)`. Implemented unconditionally for every applied promotion (not gated on `usageLimit` being set) so `usageCount` is an accurate redemption counter for all promos, matching the `evaluator.ts:71-78` `usageLimit` gate which only reads it. No new test (single DB `UPDATE`, no new logic branch; gating already covered by `promotion-evaluator.test.ts`). Verified: typecheck packages/services clean, scoped Biome clean, 678/678 services tests PASS (1 pre-existing flaky timeout in `whistleblower-anonymity.test.ts` under full-suite load, passes in isolation, unrelated to this change). Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
...
| T-0303 | KDS staff board + customer display UI (G2, T-0299 Finding 3 remediation) | Claude Sonnet 4.6 | 2026-06-12 | 010227b | Staff board `/kitchen` (tap-to-advance queued→making→ready→served, gated `kitchen.view`) + customer display `/kitchen-display/[locationId]` via new SSE route `/api/kitchen/display/[locationId]` backed by `display-service.ts`; nav/sidebar entries + i18n ×3; `buildProductSummary` now resolves localized product names; `cashier` role granted `kitchen.view`. typecheck/lint/permission-lint/i18n/678-678 tests PASS; browser verification blocked by local DB connectivity (`ETIMEDOUT 103.93.162.50:5432`, external infra issue). Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
| T-0304 | Modifier picker UI: sugar/ice/topping (G1, T-0299 Finding 1 remediation, ADR-0019) | Claude Sonnet 4.6 | 2026-06-13 | e6ba8f3 | New `groupRole` column on `product_modifier_groups` (migration 0045, not yet applied) + canonical `ModifierSelection[]` shape for `salesOrderLines.modifierJson` via new `@erp/shared/pos/modifiers` helpers; new `ModifierPickerModal` wired into `product-search.tsx`; `order-cart.tsx` shows selected options; `kds-service.buildProductSummary` + Naixer `normalizeNaixerModifiers` + both label-printing views grouped by `groupRole`; price validation includes modifier `extraPrice`; offline/demo IndexedDB (`indexeddb.ts` v2, `demo-db.ts`/`demo-master.ts` v2) extended to snapshot modifier groups. i18n ×3 (`pos.modifierPicker.*`). typecheck (db/shared/services/mcp/web/offline) clean, `pnpm lint:permissions` PASS, i18n parity PASS (4759 keys), `@erp/shared` 91/91 + `@erp/services` kitchen 31/31 + pos 102/102 PASS; browser verification blocked by local DB connectivity (`ETIMEDOUT 103.93.162.50:5432`, external infra issue). Checkpoint: `docs/checkpoints/T-0299-fnb-erp-gap-audit.checkpoint.md`. |
| T-0305 | Two cross-cutting bugfixes from external `E:\erp-benchmark` analysis (`COMPREHENSIVE_ANALYSIS_SUMMARY.md` verified bugs #1-#2) | Claude Sonnet 4.6 | 2026-06-13 | 953b70b | (1) `accounting/invoice.ts` `createInvoice`: line tax was `(subtotal * taxRate) / 10000n` (truncating division, e.g. Rp123,456 @ 10% → 12,345 instead of correctly-rounded 12,346) — now uses existing `@erp/shared/money` `calculateExclusiveTax` (round-half-up). (2) `hr/leave-service.ts` `approveLeave`: check-then-act race — two concurrent approvals of the same leave request could both pass the `status !== 'pending'` SELECT-check and both mutate `leaveBalances` (double-decrement `pendingDays`/double-increment `usedDays`); now claims via conditional `UPDATE ... WHERE status='pending' RETURNING id`, returns `AppError.conflict('hr.leave.not_pending')` if already claimed — same pattern as `update-employee.ts`/`attendance-service.ts`. Added `packages/shared/tests/money.test.ts` (39 tests) covering `calculateExclusiveTax`/`extractInclusiveTax`/`multiply`/`divide` rounding to lock in the round-half-up behavior the invoice fix relies on. Bug #3 (GRN zero-cost JE) determined to be a business-policy question, not a pure logic bug (zod schema already allows `unitPrice='0'` for free goods/samples) — backlogged as **G16** in `docs/benchmark/fnb-erp-gap-analysis.md` Part D pending Lintang decision. `pnpm --filter @erp/services typecheck` PASS, `pnpm --filter @erp/shared test -- money` 39/39 PASS, Biome clean on 3 changed files. Checkpoint: `docs/checkpoints/T-0305-bugfix-tax-rounding-leave-race.checkpoint.md`. |
| T-0306 | Triage `FUNCTIONAL_BUG_AUDIT.md` CRITICAL #1-4 against current code (docs-only, no code changes) | Claude Sonnet 4.6 | 2026-06-13 | _pending_ | Cross-verified all 4 CRITICAL findings against actual source: **#1** (Invoice Payment Routes to Wrong Account) — false positive, `payInvoice`'s `partnerLine` match predicate structurally cannot match an income/expense line. **#2** (Refund Amount Can Exceed Original Payment) — false positive, `lineRefundAmount = lineTotal*qty/originalQty` is bounded by `lineTotal` given already-enforced `qty<=originalQty`. **#3** (Donation Amount Can Exceed Tendered Cash) — false positive, `normalizeSalePayments` maintains a provable `donationRemaining<=cashRetainedRemaining` invariant and the final `cashRetainedRemaining>0` guard is the missing check the audit asked for. **#4** (Sale Duplicate Prevention Fails Under Load) — false positive, `claimIdempotency` already uses `INSERT...ON CONFLICT DO NOTHING...RETURNING` on a unique `(idempotencyKey, locationId)` constraint, exactly the audit's recommended fix. While verifying #1, found a real separate latent bug in `postInvoice` — never emits a tax JE line, so any invoice with `taxAmount>0` fails `createJournal`'s balance check — backlogged as **G17** (P0) in `docs/benchmark/fnb-erp-gap-analysis.md` Part D, gated on a Lintang decision (tax-code mapping for manual invoices). Full triage log in Part F of the same doc. Checkpoint: `docs/checkpoints/T-0306-bug-audit-triage-critical.checkpoint.md`. |
...
> 2. Ambil kartu paling atas yang terbuka pada fase yang sedang berjalan; **urutan fase = F0 → F1 → F2 → F3 → F7 → F4 → F5 → F8 → F6** (lihat tabel §3 master plan; F7 native-print dan F8 API publik disisipkan T-0288), dilarang loncat. Kartu F0–F1 di master plan §4–§5; kartu fase lain di `docs/plans/cards/F*.md`. SEMUA kartu sudah dipecah — jangan memecah/menyusun kartu sendiri.
...
### 🍵 Backlog T-0299 — F&B POS/Kitchen/Promotion gap remediation (audit lengkap: `docs/benchmark/fnb-erp-gap-analysis.md`)

> Temuan baru dari audit dual-lens (presence + logic) yang BELUM tercakup kartu F0-F8 manapun — fokus breadth+correctness POS/restoran, bukan platform/enterprise depth. Boleh disisipkan sebagai "hotfix kecil" di sela fase master plan yang berjalan (terutama G3a/G4/G8/G9 yang self-contained, S/XS effort), kartu besar (G1/G2) tunggu jendela kerja khusus. Cetak `T-NNNN` baru saat mengambil salah satu.

| Kode | Title | Priority | Effort | Catatan |
|----|-------|----------|--------|---------|
| ~~G3a~~ | ~~Promotion evaluator: implementasikan `buy_x_get_y` + `free_item` sebagai diskon per-line + test baru~~ | **P0** | S | ✅ **DONE — T-0300**. `evaluator.ts` kini punya `applyGetItemBenefit()`; 13 test baru di `promotion-evaluator.test.ts`, 678/678 PASS, typecheck clean. |
| G3b | Promo `complimentary`: routing GL ke `expenseAccountCode` (perlu ADR kecil) | P1 | M | Tergantung G3a (✅ done) + ADR akuntansi. |
| ~~G15~~ | ~~Increment `promotions.usageCount` setelah promo diterapkan ke sale — `usageLimit` saat ini tidak pernah mengikat~~ | **P1** | S | ✅ **DONE — T-0302**. `create-sale.ts` kini `UPDATE promotions SET usage_count = usage_count + 1 WHERE id IN (...)` untuk setiap `promotionId` unik yang diterapkan, dijalankan setelah jurnal terposting & idempotency tersimpan (lewat semua titik rollback). typecheck + 678/678 services test PASS. |
| ~~G4~~ | ~~Toggle "86" produk (`isAvailable`/`is86dAt`) + tombol POS + auto-grey di `product-search.tsx`~~ | **P0** | S | ✅ **DONE — T-0301**. Kolom `is_available`/`is_86d_at` (migrasi 0044) + service `setProductAvailability` (audit `update`) + tombol eye/eye-slash di `product-search.tsx` (gated `inventory.product.update`) + MCP `inventory.set_product_availability`. i18n ×3. typecheck/lint/permission-lint clean. |
| ~~G1~~ | ~~Modifier picker UI (gula/es/topping) — full Finding 1 remediation~~ | **P0** | L | ✅ **DONE — T-0304**. ADR-0019 (`groupRole` di `product_modifier_groups`, migrasi 0045) + canonical `ModifierSelection[]` + `ModifierPickerModal` di `product-search.tsx` + KDS/Naixer/label/offline-demo sync semua dirombak ke `groupRole`. |
| ~~G2~~ | ~~KDS staff board + customer display UI — full Finding 3 remediation~~ | **P0** | L | ✅ **DONE — T-0303**. Staff board `/kitchen` + customer display `/kitchen-display/[locationId]` + SSE route + nav/sidebar + i18n ×3. |
| G8 | Open/misc sale line (deskripsi+harga bebas) di POS | P1 | S | Quick win. |
| G9 | Auto-expiry status promo (`active`→`expired` saat `endsAt` lewat) — kosmetik tapi membingungkan di Settings | P2 | XS | Quick win kosmetik. |
| G5 | Rekonsiliasi settlement delivery channel vs payout platform | P1 | M | Komisi 80% sudah dihitung, rekonsiliasi belum ada. |
| G10 | Audit + (jika perlu) bangun cash count denominasi akhir shift + X/Z report | **P1 (audit dulu)** | M | Kemungkinan celah kontrol kas nyata, bukan kosmetik. |
| G6 | Tabel `reservations` — keep (bangun §3.1 floor plan) atau remove — **perlu keputusan Lintang** | P2 | tergantung | Lihat Finding 4. |
| G7 | Penjadwalan maintenance aset/peralatan (log + reminder) | P2 | M | §16 — saat ini hanya register depresiasi aset tetap. |
| G11 | Bundel peningkatan struk: saldo loyalty, footer/promo, bahasa sesuai pelanggan, struk digital WA/email | P2 | M | Audit dulu — beberapa item mungkin sudah ada. |
| G12 | Quick-key/favorit di grid POS | P2 | S | Kosmetik, quick win. |
| G13 | Combo/meal-deal builder | ⬜ audit dulu | M-L | Belum diverifikasi ada/tidak. |
| G14 | Service charge line item (on/off + %) | ⬜ audit dulu, P1 jika dikonfirmasi perlu | S-M | Beda dari fitur donasi/pembulatan yang sudah ada. |

**Keputusan bisnis tertunda dari Lintang** (lihat Part D gap-analysis untuk detail): (a) apakah ada outlet dengan dine-in/table service → menentukan §3.1 + G6; (b) perlu modul Project/Task internal (§15)?; (c) kebutuhan checklist QC/food-safety konkret (§17); (d) apakah Aroadri pernah pakai tip/service charge (§3.4).
