...
|----|-------|-------|---------|-------------|--------|------|
| T-0187 | Lengkapi panduan user ERP + mekanisme refresh konten docs DB per bahasa | Codex | 2026-05-25 22:53 WIB | 2026-05-25 22:53 WIB | IN_PROGRESS | User minta baca panduan saat ini, jalankan subagent untuk memetakan halaman/modul/tombol/use case, lengkapi panduan, dan desain cara mengganti isi panduan di database untuk ID/EN/ZH saat seed tidak overwrite. |
...
| T-0167 | Production readiness audit and critical fixes | Codex | 2026-05-15 13:03 | 2026-05-25 22:33 WIB (Codex) | IN_PROGRESS | Maintenance fix for production build failure in `/reporting/aging-payables`: replaced `"use server"` re-export with async wrapper. Local `pnpm --filter @erp/web build` and `pnpm -r build` PASS. |
