...
export function OpnameLineTable({ lines, status, sessionId }: Props) {
  ...
  async function handleSaveLine(lineId: string, productId: string, variantId: string | null | undefined) {
  ...
  async function handleSaveAll() {
    const dirty = Array.from(dirtyLines.entries()).map(([lineId, _countedQty]) => {
      const line = optLines.get(lineId);
      if (!line) return null;
      return {
        productId: line.productId,
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-brand-cream-3 bg-brand-cream-1">
              ...
              <th className="px-4 py-2.5 text-center text-xs font-semibold text-brand-ink-2">Satuan</th>
              <th className="px-4 py-2.5 text-right text-xs font-semibold text-brand-ink-2">Stok Sistem</th>
              <th className="px-4 py-2.5 text-right text-xs font-semibold text-brand-ink-2">Dihitung</th>
              <th className="px-4 py-2.5 text-right text-xs font-semibold text-brand-ink-2">Selisih</th>
              {isReadOnly && (
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-brand-ink-2">Nilai Selisih</th>
              )}
              <th className="px-4 py-2.5 text-center text-xs font-semibold text-brand-ink-2">Status</th>
              {isEditable && <th className="px-4 py-2.5 text-center text-xs font-semibold text-brand-ink-2">Aksi</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-brand-cream-2">
            {lines.length === 0 ? (
              <tr>
                <td colSpan={isEditable ? 9 : 9} className="px-4 py-12 text-center text-brand-ink-3">
                  Belum ada baris produk.
                </td>
              </tr>
            ) : (
              Array.from(optLines.values()).map((line) => {
                const counted = line.countedQty ? parseFloat(line.countedQty) : NaN;
                const system = line.systemQty ? parseFloat(line.systemQty) : NaN;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-mono text-xs text-brand-ink-2">{line.productId.slice(0, 12)}</td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {isEditable ? (
                        ...
                        <span className={`font-medium ${line.isCounted ? 'text-brand-ink' : 'text-brand-ink-3'}`}>
                    ...
                    {/* Variance */}
                    <td className={`px-4 py-2.5 text-right font-semibold ${varianceClass(variance !== null ? String(variance) : null)}`}>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {line.isCounted ? (
                        ...
                        <span className="inline-flex items-center gap-1 rounded-full bg-brand-cream-2 px-2 py-0.5 text-xs font-medium text-brand-ink-3">—</span>
                      )}
                    </td>
