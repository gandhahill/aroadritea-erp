          ...
          <div key={step.key} className="flex items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:block">
                <p
                  className={`text-xs font-semibold ${active ? 'text-brand-ember-5' : done ? 'text-brand-jade' : 'text-brand-ink-3'}`}
                >
                  {step.label}
                </p>
            ...
            {idx < STEPS.length - 1 && (
              <div
                className={`mx-2 h-px w-8 flex-shrink-0 sm:mx-3 sm:w-12 ${done ? 'bg-brand-jade' : 'bg-brand-cream-2'}`}
              />
            )}
          </div>
        );
