  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(['', 'open', 'in_progress', 'waiting_reporter', 'resolved', 'closed'] as const).map((s) => (
          <Link
            key={s || 'all'}
              ...
              result.items.map((row) => (
                <tr key={row.id} className="border-t border-brand-cream-3">
                  ...
                  <td className="px-3 py-2">
                    <PriorityBadge p={row.priority} label={t(`priority.${row.priority}` as 'priority.normal')} />
                  </td>
                  <td className="px-3 py-2">
                    <StatusBadge s={row.status} label={t(`status.${row.status}` as 'status.open')} />
                  </td>
