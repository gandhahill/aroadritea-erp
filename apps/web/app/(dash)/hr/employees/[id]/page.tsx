...
import { employees, employmentContracts, attendance, leaveBalances, leaveTypes, leaveRequests } from '@erp/db/schema/hr';
...
export default async function EmployeeDetailPage({
  ...
  const [lateRows] = await db
    ...
    .from(attendance)
    .where(and(eq(attendance.employeeId, id), eq(attendance.tenantId, tenantId), eq(attendance.isLate, true)));
  ...
  const statusCfg = STATUS_COLOR[emp.status] ?? { bg: 'bg-brand-cream-2', text: 'text-brand-ink-2' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${statusCfg.bg} ${statusCfg.text}`}>
            {statusLabel}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 rounded-xl border border-brand-cream-3 bg-card p-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-brand-ink-3">{emp.position}{emp.department ? ` — ${emp.department}` : ''}</p>
          <p className="text-xs text-brand-ink-3">NIK: {emp.nik} · {emp.email}</p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-brand-cream-3 bg-card p-6">
          ...
          <dl className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
            ...
            <div>
              ...
              <dd className="font-medium text-brand-ink">{emp.emergencyContactName ?? '—'}{emp.emergencyContactPhone ? ` — ${emp.emergencyContactPhone}` : ''}</dd>
            </div>
          </dl>
        </div>
        ...
        <div className="rounded-xl border border-brand-cream-3 bg-card p-6">
          ...
          {contracts.length === 0 ? (
            ...
            <div className="space-y-3">
              {contracts.map((c) => (
                <div key={c.id} className="flex items-center justify-between rounded-lg border border-brand-cream-2 bg-brand-cream-1 px-4 py-3">
                  <div>
                    <div className="text-sm font-medium text-brand-ink">
                      {c.contractType === 'pkwt' ? 'PKWT (Kontrak)' : c.contractType === 'pkwtt' ? 'PKWTT (Tetap)' : c.contractType}
                  ...
                  <div className="text-right">
                    <div className="text-sm font-semibold text-brand-ember-5">{formatMoney(c.baseSalary)}</div>
        ...
        <div className="rounded-xl border border-brand-cream-3 bg-card p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="rounded-lg border border-brand-cream-3 bg-brand-cream-1 p-4 text-center">
              <div className="text-2xl font-bold text-brand-ink">{emp.probationEndDate ? formatDate(emp.probationEndDate) : '—'}</div>
          ...
          {balances.length === 0 ? (
            ...
            <div className="mb-6 grid grid-cols-3 gap-3">
              {balances.map((b) => {
                ...
                const total = parseFloat(String(b.totalDays)) || 0;
                const used = parseFloat(String(b.usedDays)) || 0;
                const pending = parseFloat(String(b.pendingDays)) || 0;
                const remaining = Math.max(0, total - used - pending);
                return (
                  <div key={b.leaveTypeId} className="rounded-lg border border-brand-cream-3 bg-brand-cream-1 p-4">
                    <div className="text-xs font-medium text-brand-ink-2">{name?.id ?? b.leaveTypeCode ?? 'Unknown'}</div>
            ...
            <>
              ...
              <div className="space-y-2">
                {recentRequests.map((r) => {
                  ...
                  const statusColor = r.status === 'approved' ? 'text-brand-jade' : r.status === 'rejected' ? 'text-rose-500' : 'text-brand-gold';
                  return (
                    <div key={r.id} className="flex items-center justify-between rounded-lg border border-brand-cream-2 px-4 py-2 text-sm">
                      <span className="text-brand-ink">{reqName?.id ?? r.leaveTypeCode}</span>
                      <span className="text-brand-ink-3">{formatDate(r.startDate)} — {formatDate(r.endDate)}</span>
