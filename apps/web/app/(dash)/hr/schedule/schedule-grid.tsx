...
export function ScheduleGrid({
  ...
  function getSwapCandidates(assignment: RosterAssignment) {
    return options.employees.filter(
      (employee) =>
        employee.id !== assignment.employeeId && employee.locationId === assignment.locationId,
    );
  }

  function resolveActionError(error: string | undefined, fallback: string): string {
    if (!error) return fallback;
    const prefix = 'hr.schedule.';
    if (error.startsWith(prefix)) return t(error.slice(prefix.length));
    return error;
  }
  ...
  function confirmSwapAssignment() {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        setErr(resolveActionError(response.error, t('errors.swapFailed')));
        return;
      }
      setAssignments((prev) =>
        prev.map((assignment) =>
  ...
  async function toggle(
    ...
    startTransition(async () => {
      if (existing) {
        ...
        if (!response.ok) {
          setErr(resolveActionError(response.error, t('errors.deleteFailed')));
          return;
        }
      ...
      if (!response.ok || !response.id) {
        setErr(resolveActionError(response.error, t('errors.saveFailed')));
        return;
      }
                  ...
                  return (
                    <td key={date} className="px-2 py-2 align-top">
                      <div className="flex flex-col gap-1">
                        {options.shifts
                          .filter((shift) => shift.locationId === employee.locationId)
                          .map((shift) => {
