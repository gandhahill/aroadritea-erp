/**
 * Password reset request form for member portal.
 */
'use client';
...
import { requestPasswordResetAction } from '../actions/member';
...
export function PasswordResetRequestForm({ locale }: Props) {
  const t = useTranslations('member.passwordReset.request');
  ...
  return (
    <section className="mx-auto max-w-md px-4 py-16 sm:px-6">
      <div className="rounded-2xl border border-brand-red/10 bg-brand-cream-1 p-8 shadow-soft">
        ...
        {sent ? (
          <div className="mt-6 rounded-lg border border-brand-jade/20 bg-brand-jade/10 px-4 py-4 text-sm text-brand-ink-2">
            {t('sent')}
          </div>
        ) : (
        ...
        <button
          ...
          className="mt-6 w-full text-center text-sm font-semibold text-brand-red hover:underline"
        >
          {t('backToLogin')}
        </button>
      </div>
